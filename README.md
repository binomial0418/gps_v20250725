...
| WiFi SSID | WiFi 網路名稱 | - |
| WiFi 密碼 | WiFi 網路密碼 | - |
| 發佈移動門檻（低速） | 低速時的移動距離門檻 | 50 公尺 |
| 發佈移動門檻（高速） | 高速時的移動距離門檻 | 100 公尺 |
| 高速門檻 | 判斷高速/低速的速度分界 | 70 km/h |
| 位置更新間隔 | 定期檢查間隔 | 1500 毫秒 |
| 方向角變化門檻 | 方向改變多少度才觸發 | 25 度 |
| 路徑轉向角門檻 | 路徑夾角多少度才觸發 | 30 度 |
| 靜止速度門檻 | 低於此速度視為靜止 | 3 km/h |
...
| 欄位 | 說明 | 格式 |
|------|------|------|
| `_type` | 訊息類型（固定為 "location"） | 字串 |
| `tst` | Unix 時間戳（系統時間） | Unix timestamp |
| `lat` | 緯度 | 浮點數（6 位小數） |
| `lon` | 經度 | 浮點數（6 位小數） |
| `acc` | 精度 | 公尺 |
| `vel` | 速度 | km/h |
| `cog` | 方向角（Course Over Ground） | 度（0-360°） |
| `satcnt` | 衛星數量 | 整數 |
| `gpstime` | GPS 時間（精確時刻） | HHMMSS.CC 格式（HH:MM:SS.centisecond） |

### 時間欄位說明

- **`tst`**：來自 NTP 伺服器的系統時間，精確到秒，用於記錄資料的絕對時間
- **`gpstime`**：來自 GPS 衛星的 UTC 時間，精確到 1/100 秒（百分之一秒），與位置資料完全同步

**時間格式範例：**
- `143052.75` = 14:30:52.75 (下午 2:30:52.75）
- `000000.00` = 無效或未取得 GPS 時間
...
程式會根據以下 **4 種觸發條件** 記錄軌跡點：

#### 1️⃣ **即時方向角觸發** ⚡
當 GPS 方向角**快速變化**時立即記錄（無需等待更新間隔）

**觸發條件：**
- GPS 定位有效且方向角有效
- 方向角變化 ≥ `COURSE_THRESHOLD_DEG`（預設 25°）
- 當前速度 > `SPEED_THRESHOLD_KMPH`（預設 3 km/h）
- 衛星數 ≥ `MIN_SATELLITES`（預設 4 顆）
...
**觸發條件：**
- 定期檢查（每 `UPDATE_INTERVAL_MS` = 1500ms）
- 已有前一個記錄點
- 移動距離 ≥ 動態門檻：
  - 低速（速度 < 70 km/h）：≥ `MOVE_THRESHOLD_METERS`（預設 50 m）
  - 高速（速度 ≥ 70 km/h）：≥ `MOVE_THRESHOLD_METERS2`（預設 100 m）

**特點：** 根據速度自動調整距離門檻，直線路段不過度記錄
```
🚶‍♂️ 移動觸發：距離 X.XX m (>= 50 m [低速])
🚶‍♂️ 移動觸發：距離 X.XX m (>= 100 m [高速])
```
...
- 定期檢查（每 `UPDATE_INTERVAL_MS` = 1500ms）
- 已有前一個記錄點
- 距上次發布時間 ≥ `TIME_THRESHOLD_MS`（預設 30 秒）

**特點：** 確保長時間靜止或移動緩慢時仍會更新位置
```
⏰ 時間觸發：經過 X.X 秒 (>= 30 s)
```
...
- 已有前兩個記錄點
- 當前速度 > `SPEED_THRESHOLD_KMPH`（預設 3 km/h）
- 路徑轉向角 ≥ `PATH_TURN_THRESHOLD_DEG`（預設 30°）

**計算方式：**
```
方向1 = 再前一個記錄點 → 上一個記錄點 的方向
方向2 = 上一個記錄點 → 當前點 的方向
轉向角 = 方向1 和 方向2 的夾角
...
| 情況 | 原因 |
|------|------|
| **GPS 定位無效** | 需要 `gps.location.isValid() == true` |
| **衛星數不足** | 需要至少 4 顆衛星 |
| **車輛靜止超時** | 速度 ≤ 3 km/h 超過 60 秒 → 進入靜止模式，停止上傳 |
| **第一個點** | 沒有前一個位置參考，只記錄首次方向角 |
| **無符合任何條件** | 距離、時間、方向、轉向角都未達各自門檻 |

---

### 可配置參數一覽

#### 透過網頁設定介面調整（即時生效，無需重新編譯）

| 參數 | 預設值 | 單位 | 說明 | 相關功能 |
|------|--------|------|------|---------|
| WiFi SSID | - | - | WiFi 網路名稱 | 連線設定 |
| WiFi 密碼 | - | - | WiFi 網路密碼 | 連線設定 |
| 發佈移動門檻（低速） | 50 | 公尺 | 低速時的移動距離門檻 | 距離觸發 |
| 發佈移動門檻（高速） | 100 | 公尺 | 高速時的移動距離門檻 | 距離觸發 |
| 高速門檻 | 70 | km/h | 判斷高速/低速的速度分界 | 距離觸發 |
| 位置更新間隔 | 1500 | 毫秒 | 定期檢查間隔（距離、時間、轉向） | 定期觸發 |
| 方向角變化門檻 | 25 | 度 | 單點方向角變化達此值立即記錄 | 即時方向角觸發 |
| 路徑轉向角門檻 | 30 | 度 | 相鄰路徑夾角達此值記錄 | 路徑轉向角觸發 |
| 靜止速度門檻 | 3 | km/h | 低於此速度視為靜止 | 靜止模式 |

#### 固定參數（需編輯程式碼）

若需調整以下參數，請編輯 `gps_v20250725.ino` 並重新編譯上傳：

| 參數 | 預設值 | 單位 | 說明 |
|------|--------|------|------|
| `TIME_THRESHOLD_MS` | 30000 | 毫秒 | 時間觸發門檻 |
| `IDLE_TIMEOUT_MS` | 60000 | 毫秒 | 進入靜止模式的超時時間 |
| `MIN_SATELLITES` | 4 | 顆 | 最小衛星數量（小於此值不記錄） |
| `BATCH_SIZE` | 3 | 筆 | 累積多少筆資料後啟動批次上傳 |
| `UPLOAD_INTERVAL_MS` | 1000 | 毫秒 | 每筆資料的 MQTT 發布間隔 |
...
### v2.1.0 (2025-11-26)
- ✨ 新增路徑轉向角觸發條件（捕捉蜿蜒路段）
- ✨ 新增 GPS 時間欄位（gpstime）到 MQTT 訊息
- ✨ 實現動態距離門檻（根據速度自動切換 50m/100m）
- ✨ 新增即時方向角檢查（不受更新間隔限制）
- 📚 完整文檔化所有軌跡記錄條件
- 🐛 修復括號匹配問題
